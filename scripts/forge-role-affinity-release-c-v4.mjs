...
replaceOnce("__tests__/matchingScore.test.ts", `  const [k0, k1, k2, k3] = KINKS;`, `  const [k0, k1, k2, k3] = ["latex_rubber", "lingerie", "uniforms", "feet"]\n    .map((id) => KINKS.find((kink) => kink.id === id)!);`);
...
]) replaceAllIfPresent(questionnaire, `"${oldId}"`, `"${giveId}"`);
...
console.log("Release C tests gebruiken expliciete semantiek en stabiele generieke fixtures.");
