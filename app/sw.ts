...
self.addEventListener("notificationclick", (event) => {
  if (!event.notification.tag.startsWith("intimacy-reminder-")) return;
  ...
  event.waitUntil((async () => {
    ...
    if (existing) {
      const windowClient = existing as WindowClient;
      await windowClient.navigate("/intimacy");
      await windowClient.focus();
      return;
    }

    await self.clients.openWindow("/intimacy");
  })());
});
