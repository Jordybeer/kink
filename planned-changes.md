...
- 2026-07-11: session handoff, clean state — dev tree clean, 227 tests + build green. The contract-PDF arc (signature spacing, unified four-section table, choice-ladder ordering, comment bullets) is **complete**; a fresh session should not re-touch `app/contract/page.tsx` PDF code and should start at Phase 31. Visual PDF checks use the standalone jsPDF-repro + `pdftoppm` pattern (import `jspdf/dist/jspdf.node.min.js` by absolute path in a scratchpad `.mjs`).
