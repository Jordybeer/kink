    ...
    <>
      {model.summary.jointlyAssessed === 0 && (
        <div className="mb-4 rounded-xl border px-4 py-5 text-center" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
          <p className="text-sm font-medium" style={{ color: "var(--text)" }}>Nog geen voorkeuren aan beide kanten ingevuld.</p>
          <p className="mt-1 text-xs" style={{ color: "var(--text2)" }}>Een antwoord van één profiel alleen wordt niet als vergelijking geïnterpreteerd.</p>
        </div>
      )}
      ...
      {CATEGORIES.map((category) => {
        ...
        return (
          <section key={category} id={`cat-${category}`} className="mb-6" style={{ scrollMarginTop: "calc(var(--nav-h) + var(--compare-subnav-h) + 1rem)" }}>
            <h2 className="mb-2 px-1 text-sm" style={{ fontFamily: "var(--font-display, Georgia, serif)", fontStyle: "italic", fontWeight: 400, color: "var(--text)" }}>{kinkCategoryLabel(category)}</h2>
      ...
      {uncategorized.length > 0 && (
        <section className="mb-6" aria-labelledby="compare-more-heading">
          <h2 id="compare-more-heading" className="mb-2 px-1 text-sm" style={{ fontFamily: "var(--font-display, Georgia, serif)", fontStyle: "italic", fontWeight: 400, color: "var(--text)" }}>Meer</h2>
      ...
      {!filtersActive && model.unpaired.length > 0 && (
        <details className="mb-6 rounded-xl border" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
          <summary className="focus-ring min-h-11 cursor-pointer px-3 py-2.5 text-sm font-medium">Nog niet door beiden beoordeeld · {model.unpaired.length}</summary>
          <div className="space-y-2 px-3 pb-3">
            <p className="text-xs" style={{ color: "var(--text2)" }}>Deze voorkeuren tellen niet mee als vergelijking.</p>
      ...
      <div className="flex flex-col gap-2 pb-2 pt-2">
        <div className="flex gap-2">
          <Link href={`/scene?a=${profileA.id}&b=${profileB.id}`} prefetch={false} className="focus-ring flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border px-3 py-2.5 text-xs font-medium transition-opacity hover:opacity-80" style={{ borderColor: "var(--border)", color: "var(--text)" }}><FilmSlate size={14} aria-hidden="true" /> Plan een scène</Link>
          <Link href={`/contract?a=${profileA.id}&b=${profileB.id}`} prefetch={false} className="focus-ring flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-medium transition-opacity hover:opacity-80" style={{ background: "var(--action-primary)", color: "var(--on-accent)" }}><FileText size={14} aria-hidden="true" /> Contract</Link>
        </div>
        <div className="flex justify-center pt-1"><button type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="focus-ring min-h-11 rounded-full border px-4 text-xs transition-colors" style={{ color: "var(--text2)", borderColor: "var(--border)" }}><ArrowUp size={14} aria-hidden="true" /> Terug naar boven</button></div>
      </div>
    </>
  );
}
