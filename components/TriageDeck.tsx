...
function questionnairePresentation(kink: Kink) {
  ...
  return { essence: firstSentence, hasDetails: description.length > firstSentence.length + 1 };
}
            ...
            <div data-testid="question-content" className="relative z-[1] grid min-h-full gap-2" style={{ gridTemplateRows: "44px 68px 48px 40px minmax(18rem, 1fr) 80px 36px" }}>
              <div className="flex min-h-0 items-center gap-2.5">
                <button type="button" data-testid="question-category-meta" onClick={() => openCategoryExplainer(current.category)} className="focus-ring flex min-w-0 flex-1 items-center gap-2 rounded-lg text-left" aria-label={`Uitleg over ${kinkCategoryLabel(current.category)}`}>
                  <span aria-hidden="true" className="relative h-1.5 w-1.5 flex-none rounded-full" style={{ background: categoryAccent, boxShadow: `0 0 10px color-mix(in srgb, ${categoryAccent} 32%, transparent)` }}>{current.category !== "custom" && !seenCategories.has(current.category) && <span className="absolute -right-1.5 -top-1.5 h-1.5 w-1.5 rounded-full" style={{ background: "var(--accent)" }} />}</span>
                ...
                <button type="button" data-tour="curious" onClick={() => onCuriousChange(current.id, !currentEntry?.curious)} aria-pressed={!!currentEntry?.curious} aria-label={currentEntry?.curious ? "Verwijder nieuwsgierig markering" : "Markeer als nieuwsgierig"} title={currentEntry?.curious ? "Niet meer nieuwsgierig" : "Nieuwsgierig"} className="focus-ring inline-flex h-11 w-11 flex-none items-center justify-center rounded-full border transition-[transform,background-color,border-color,color] duration-150 active:scale-95 motion-reduce:active:scale-100 motion-reduce:transition-none" style={currentEntry?.curious ? { background: "color-mix(in srgb, var(--curious) 12%, var(--surface2))", borderColor: "color-mix(in srgb, var(--curious) 38%, var(--border))", color: "var(--curious)" } : { background: "color-mix(in srgb, var(--surface2) 78%, transparent)", borderColor: "color-mix(in srgb, var(--border) 90%, var(--text2))", color: "var(--text2)" }}><Star size={18} weight={currentEntry?.curious ? "fill" : "regular"} aria-hidden="true" /></button>
                <button type="button" data-tour="private" onClick={() => onPrivateChange(current.id, !currentEntry?.privateResponse)} aria-pressed={!!currentEntry?.privateResponse} aria-label={currentEntry?.privateResponse ? "Antwoord niet langer verbergen" : "Antwoord verbergen"} title={currentEntry?.privateResponse ? "Verborgen antwoord" : "Antwoord verbergen"} className="focus-ring inline-flex h-11 w-11 flex-none items-center justify-center rounded-full border transition-[transform,background-color,border-color,color] duration-150 active:scale-95 motion-reduce:active:scale-100 motion-reduce:transition-none" style={currentEntry?.privateResponse ? { color: "var(--accent)", borderColor: "color-mix(in srgb, var(--accent) 36%, var(--border))", background: "color-mix(in srgb, var(--accent) 10%, var(--surface2))" } : { color: "var(--text2)", borderColor: "color-mix(in srgb, var(--border) 90%, var(--text2))", background: "color-mix(in srgb, var(--surface2) 78%, transparent)" }}>{currentEntry?.privateResponse ? <EyeSlash size={18} weight="bold" aria-hidden="true" /> : <Eye size={18} aria-hidden="true" />}</button>
              </div>

              <div className="flex min-h-0 items-center"><h3 data-testid="question-title" className="line-clamp-2 text-[clamp(1.35rem,6.5vw,1.75rem)] leading-[1.08] tracking-[-0.018em]" style={{ fontFamily: "var(--font-display, Georgia, serif)", fontWeight: 500, color: "var(--text)" }}>{current.name}</h3></div>
              <p data-testid="question-essence" className="line-clamp-2 self-start text-[13px] leading-5" style={{ minHeight: "2.5rem", color: "var(--text2)" }}>{presentation.essence}</p>

              <div data-testid="question-detail-slot" className="grid min-h-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
                {presentation.hasDetails && <button type="button" onClick={() => setDetailKinkId(current.id)} aria-haspopup="dialog" aria-expanded={detailOpen} className="focus-ring inline-flex h-10 min-w-0 items-center gap-2 rounded-xl px-3 text-left transition-[transform,background-color] duration-150 active:scale-[0.995] motion-reduce:active:scale-100 motion-reduce:transition-none" style={{ background: "color-mix(in srgb, var(--surface2) 66%, transparent)", color: "var(--text2)" }}><Info size={16} aria-hidden="true" style={{ color: "var(--accent-text)" }} /><span className="min-w-0 truncate text-xs font-semibold" style={{ color: "var(--text)" }}>Uitleg & voorbeelden</span><CaretRight size={14} className="ml-auto flex-none" aria-hidden="true" /></button>}
                {current.safetyNote && <button type="button" data-testid="safety-disclosure" aria-haspopup="dialog" aria-expanded={safetyOpen} onClick={() => setSafetyKinkId(current.id)} className="focus-ring inline-flex h-10 min-w-0 items-center gap-2 rounded-xl px-3 text-left transition-[transform,background-color] duration-150 active:scale-[0.995] motion-reduce:active:scale-100 motion-reduce:transition-none" style={{ background: "color-mix(in srgb, var(--surface2) 66%, transparent)", color: "var(--text2)" }}><ShieldCheck size={16} weight="duotone" style={{ color: "var(--accent-text)" }} aria-hidden="true" /><span className="text-xs font-semibold" style={{ color: "var(--text)" }}>Veiligheid</span><CaretRight size={14} aria-hidden="true" /></button>}
              </div>
