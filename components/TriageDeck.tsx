      ...
      <div aria-live="polite" className="h-full min-h-0">
        {current ? (
          <div data-tour="kink-card" className="relative isolate h-full min-h-0 overflow-y-auto overscroll-contain rounded-[1.75rem] p-4" style={{ background: "color-mix(in srgb, var(--surface) 94%, #180f20)", border: "1px solid color-mix(in srgb, var(--border) 88%, var(--text2))", boxShadow: "0 18px 42px rgba(0, 0, 0, 0.28), inset 0 1px 0 rgba(255, 255, 255, 0.035)" }}>
            ...
            <div data-testid="question-content" className="relative z-[1] grid min-h-full gap-1.5" style={{ gridTemplateRows: "44px 56px 40px 44px minmax(14.75rem, 1fr) 76px 44px" }}>
              <div className="flex min-h-0 items-center gap-2.5">
                <button type="button" data-testid="question-category-meta" onClick={() => openCategoryExplainer(current.category)} className="focus-ring flex h-11 min-w-0 flex-1 items-center gap-2 rounded-lg text-left" aria-label={`Uitleg over ${kinkCategoryLabel(current.category)}`}>
