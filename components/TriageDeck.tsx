...
export default function TriageDeck({
  ...
  const presentation = current ? questionnairePresentation(current) : { essence: "", hasDetails: false };
  ...
  return (
    <>
      <div aria-live="polite" className="h-full min-h-0">
        {current ? (
          <div data-tour="kink-card" className="relative isolate h-full min-h-0 overflow-y-auto overscroll-contain rounded-[1.75rem] p-4" style={{ background: "color-mix(in srgb, var(--surface) 94%, #180f20)", border: "1px solid color-mix(in srgb, var(--border) 88%, var(--text2))", boxShadow: "0 18px 42px rgba(0, 0, 0, 0.28), inset 0 1px 0 rgba(255, 255, 255, 0.035)", scrollMarginTop: "calc(var(--nav-h) + 12px)" }}>
            <div aria-hidden="true" data-testid="question-ambient-glow" className="pointer-events-none absolute inset-x-0 top-0 h-40 rounded-[1.75rem]" style={{ background: `radial-gradient(circle at 18% 0%, color-mix(in srgb, ${categoryAccent} 12%, transparent), transparent 64%)` }} />
            <div data-testid="question-content" className="relative z-[1] grid min-h-full gap-2" style={{ gridTemplateRows: "44px 68px 48px 40px minmax(18rem, 1fr) 80px 36px" }}>
              <div className="flex min-h-0 items-center gap-2.5">
                <button type="button" data-testid="question-category-meta" onClick={() => openCategoryExplainer(current.category)} className="focus-ring flex min-w-0 flex-1 items-center gap-2 rounded-lg text-left" aria-label={`Uitleg over ${kinkCategoryLabel(current.category)}`}>
                  <span aria-hidden="true" className="relative h-1.5 w-1.5 flex-none rounded-full" style={{ background: categoryAccent, boxShadow: `0 0 10px color-mix(in srgb, ${categoryAccent} 32%, transparent)` }}>
                    {current.category !== "custom" && !seenCategories.has(current.category) && <span className="absolute -right-1.5 -top-1.5 h-1.5 w-1.5 rounded-full" style={{ background: "var(--accent)" }} />}
                  </span>
                ...
                <button type="button" data-tour="curious" onClick={() => onCuriousChange(current.id, !currentEntry?.curious)} aria-pressed={!!currentEntry?.curious} aria-label={currentEntry?.curious ? "Verwijder nieuwsgierig markering" : "Markeer als nieuwsgierig"} title={currentEntry?.curious ? "Niet meer nieuwsgierig" : "Nieuwsgierig"} className="focus-ring inline-flex h-11 w-11 flex-none items-center justify-center rounded-full border transition-[transform,background-color,border-color,color,box-shadow] duration-150 active:scale-95 motion-reduce:active:scale-100 motion-reduce:transition-none" style={currentEntry?.curious ? { background: "color-mix(in srgb, var(--curious) 12%, var(--surface2))", borderColor: "color-mix(in srgb, var(--curious) 38%, var(--border))", color: "var(--curious)" } : { background: "color-mix(in srgb, var(--surface2) 78%, transparent)", borderColor: "color-mix(in srgb, var(--border) 90%, var(--text2))", color: "var(--text2)" }}><Star size={18} weight={currentEntry?.curious ? "fill" : "regular"} aria-hidden="true" /></button>
                <button type="button" data-tour="private" onClick={() => onPrivateChange(current.id, !currentEntry?.privateResponse)} aria-pressed={!!currentEntry?.privateResponse} aria-label={currentEntry?.privateResponse ? "Antwoord niet langer verbergen" : "Antwoord verbergen"} title={currentEntry?.privateResponse ? "Verborgen antwoord" : "Antwoord verbergen"} className="focus-ring inline-flex h-11 w-11 flex-none items-center justify-center rounded-full border transition-[transform,background-color,border-color,color,box-shadow] duration-150 active:scale-95 motion-reduce:active:scale-100 motion-reduce:transition-none" style={currentEntry?.privateResponse ? { color: "var(--accent)", borderColor: "color-mix(in srgb, var(--accent) 36%, var(--border))", background: "color-mix(in srgb, var(--accent) 10%, var(--surface2))" } : { color: "var(--text2)", borderColor: "color-mix(in srgb, var(--border) 90%, var(--text2))", background: "color-mix(in srgb, var(--surface2) 78%, transparent)" }}>{currentEntry?.privateResponse ? <EyeSlash size={18} weight="bold" aria-hidden="true" /> : <Eye size={18} aria-hidden="true" />}</button>
              </div>

              <div className="flex min-h-0 items-center"><h3 data-testid="question-title" className="line-clamp-2 text-[1.75rem] leading-[1.08] tracking-[-0.018em]" style={{ fontFamily: "var(--font-display, Georgia, serif)", fontWeight: 500, color: "var(--text)" }}>{current.name}</h3></div>
              <p data-testid="question-essence" className="line-clamp-2 self-start text-sm leading-5" style={{ minHeight: "2.5rem", color: "var(--text2)" }}>{presentation.essence}</p>

              <div data-testid="question-detail-slot" className="flex min-h-0 items-center gap-2">
                {presentation.hasDetails && <button type="button" onClick={() => setDetailKinkId(current.id)} aria-haspopup="dialog" aria-expanded={detailOpen} className="focus-ring inline-flex h-10 min-w-0 flex-1 items-center gap-2 rounded-xl px-3 text-left transition-[transform,background-color] duration-150 active:scale-[0.995] motion-reduce:active:scale-100 motion-reduce:transition-none" style={{ background: "color-mix(in srgb, var(--surface2) 66%, transparent)", color: "var(--text2)" }}><Info size={16} aria-hidden="true" style={{ color: "var(--accent-text)" }} /><span className="truncate text-xs font-semibold" style={{ color: "var(--text)" }}>Uitleg & voorbeelden</span><CaretRight size={14} className="ml-auto flex-none" aria-hidden="true" /></button>}
                {current.safetyNote && <button type="button" data-testid="safety-disclosure" aria-haspopup="dialog" aria-expanded={safetyOpen} onClick={() => setSafetyKinkId(current.id)} className="focus-ring inline-flex h-10 min-w-0 flex-1 items-center gap-2 rounded-xl px-3 text-left transition-[transform,background-color] duration-150 active:scale-[0.995] motion-reduce:active:scale-100 motion-reduce:transition-none" style={{ background: "color-mix(in srgb, var(--surface2) 66%, transparent)", color: "var(--text2)" }}><ShieldCheck size={16} weight="duotone" style={{ color: "var(--accent-text)" }} aria-hidden="true" /><span className="truncate text-xs font-semibold" style={{ color: "var(--text)" }}>Veiligheid</span><CaretRight size={14} className="ml-auto flex-none" aria-hidden="true" /></button>}
              </div>
              ...
              <section className="min-h-0 border-t pt-2" style={{ borderColor: "color-mix(in srgb, var(--border) 88%, transparent)" }}>
                <div className="mb-1.5 flex items-baseline justify-between gap-3"><p className="text-xs font-semibold" style={{ color: "var(--text)" }}>Afspraken</p><span className="text-xs" style={{ color: "var(--text2)" }}>optioneel</span></div>
                <div className="grid grid-cols-2 gap-2">
                  {AGREEMENTS.map((agreement) => {
                    const active = currentEntry?.tags?.includes(agreement.value) ?? false;
                    return <button key={agreement.value} type="button" data-tour={agreement.tour} onClick={() => toggleAgreement(agreement.value)} aria-pressed={active} className="focus-ring inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl px-2.5 text-xs font-semibold transition-[transform,background-color,border-color,color,box-shadow] duration-150 active:scale-[0.99] motion-reduce:active:scale-100 motion-reduce:transition-none" style={active ? { color: agreement.emphasized ? "var(--accent)" : "var(--text)", background: agreement.emphasized ? "color-mix(in srgb, var(--accent) 10%, var(--surface2))" : "color-mix(in srgb, var(--surface3) 82%, transparent)", border: `1px solid ${agreement.emphasized ? "color-mix(in srgb, var(--accent) 34%, var(--border))" : "color-mix(in srgb, var(--text2) 20%, var(--border))"}` } : { color: "var(--text2)", background: "color-mix(in srgb, var(--surface2) 76%, transparent)", border: "1px solid color-mix(in srgb, var(--border) 92%, var(--text2))" }}>{active ? <Check size={13} weight="bold" aria-hidden="true" /> : <Circle size={13} aria-hidden="true" />}{agreement.label}</button>;
                  })}
                </div>
              </section>

              <div className="flex min-h-0 items-center gap-3"><span data-testid="question-progress" className="min-w-0 flex-1 truncate text-xs tabular-nums" style={{ color: "var(--text2)" }}>{progressLabel ?? `${totalDone} van ${kinks.length} beoordeeld`}</span><button type="button" onClick={() => skip(current)} className="focus-ring inline-flex h-9 items-center gap-1 rounded-full px-3 text-xs transition-[transform,background-color,color] duration-150 active:scale-[0.98] motion-reduce:active:scale-100 motion-reduce:transition-none" style={{ color: "var(--text2)", background: "color-mix(in srgb, var(--surface2) 66%, transparent)" }}>Later <ArrowRight size={13} aria-hidden="true" /></button></div>
            </div>
          </div>
        ) : (
      ...
      {current && presentation.hasDetails && <Sheet open={detailOpen} onClose={() => setDetailKinkId(null)} scrollable aria-label={`Uitleg en voorbeelden bij ${current.name}`}><SheetContent className="overflow-y-auto overscroll-contain px-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-3" style={{ maxHeight: "min(calc(var(--visual-viewport-height, 100dvh) - 1rem), 36rem)" }}><div className="flex items-center gap-2"><Info size={20} weight="duotone" style={{ color: "var(--accent-text)" }} aria-hidden="true" /><h3 className="text-lg font-semibold" style={{ color: "var(--text)" }}>Uitleg & voorbeelden</h3></div><p className="mt-1 text-xs" style={{ color: "var(--text2)" }}>{current.name}</p><p className="mt-4 text-sm leading-6" style={{ color: "var(--text)" }}>{current.description}</p>{current.aliases && current.aliases.length > 0 && <div className="mt-4"><p className="text-xs font-semibold" style={{ color: "var(--text2)" }}>Ook bekend als</p><p className="mt-1 text-sm leading-6" style={{ color: "var(--text)" }}>{current.aliases.join(" · ")}</p></div>}<button type="button" onClick={() => setDetailKinkId(null)} className="focus-ring mt-5 min-h-12 w-full rounded-xl text-sm font-semibold" style={{ border: "1px solid var(--border)", color: "var(--text2)" }}>Sluit</button></SheetContent></Sheet>}

      {current?.safetyNote && <Sheet open={safetyOpen} onClose={() => setSafetyKinkId(null)} scrollable aria-label={`Veiligheid bij ${current.name}`}><SheetContent className="overflow-y-auto overscroll-contain px-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-3" style={{ maxHeight: "min(calc(var(--visual-viewport-height, 100dvh) - 1rem), 32rem)" }}><div className="flex items-center gap-2"><ShieldCheck size={20} weight="duotone" style={{ color: "var(--accent-text)" }} aria-hidden="true" /><h3 className="text-lg font-semibold" style={{ color: "var(--text)" }}>Veiligheid</h3></div><p className="mt-1 text-xs" style={{ color: "var(--text2)" }}>{current.name}</p><p className="mt-4 text-sm leading-6" style={{ color: "var(--text)" }}>{current.safetyNote}</p><button type="button" onClick={() => setSafetyKinkId(null)} className="focus-ring mt-5 min-h-12 w-full rounded-xl text-sm font-semibold" style={{ border: "1px solid var(--border)", color: "var(--text2)" }}>Sluit</button></SheetContent></Sheet>}
