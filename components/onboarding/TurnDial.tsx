...
export default function TurnDial({ onComplete }: TurnDialProps) {
  ...
  function onPointerMove(event: React.PointerEvent<HTMLButtonElement>) {
    ...
    if (Math.hypot(event.clientX - startRef.current.x, event.clientY - startRef.current.y) > 5) movedRef.current = true;
  ...
  function finishPointer(event: React.PointerEvent<HTMLButtonElement>) {
    ...
    setDragging(false);
    if (!doneRef.current && rotationRef.current < COMPLETE_AT) applyRotation(0);
  }
  ...
  return (
    <div className="rounded-3xl border px-5 py-6 text-center" style={{ background: 'var(--surface)', borderColor: done ? 'var(--border-accent)' : 'var(--border)' }}>
      ...
      <button type="button" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={finishPointer} onPointerCancel={finishPointer} onClick={onClick} disabled={done} aria-label={done ? 'Geopend' : 'Draai open'} className="focus-ring relative mx-auto -mt-40 flex h-36 w-36 touch-none select-none items-center justify-center rounded-full border disabled:cursor-default" style={{ background: 'radial-gradient(circle at 35% 30%, var(--surface2), var(--surface3))', borderColor: done ? 'var(--border-accent)' : 'var(--border)', boxShadow: done ? '0 0 0 1px var(--border-accent), 0 14px 40px var(--accent-glow)' : '0 12px 32px rgba(0, 0, 0, 0.16)', transform: `rotate(${rotation}deg)`, transition: dragging ? 'none' : 'transform 300ms cubic-bezier(0.2, 0.8, 0.2, 1), border-color 200ms ease, box-shadow 200ms ease', WebkitTapHighlightColor: 'transparent' }}>
      ...
      <p className="mt-6 text-sm font-semibold" style={{ color: done ? 'var(--accent)' : 'var(--text)' }}>{done ? 'Open' : 'Draai open'}</p>
      <span className="sr-only" aria-live="polite">{done ? 'Geopend.' : 'Je kunt draaien of tikken.'}</span>
    </div>
  );
}
