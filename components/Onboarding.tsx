  ...
  return (
    <div className="relative pt-[clamp(3.5rem,8dvh,5.5rem)] text-center">
      <motion.div
        ...
        className="pointer-events-none absolute left-1/2 top-1/2 h-48 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl"
        style={{ background: 'var(--accent-glow)' }}
      />
      <motion.h1 variants={childV} className="relative text-[3.5rem] leading-none">
        <Wordmark />
      </motion.h1>
      <motion.p variants={childV} className="relative mx-auto mt-9 max-w-xs text-xl font-medium leading-8">
        Ontdek waar je voor openstaat.
      </motion.p>
      ...
      <motion.p variants={childV} className="relative mx-auto mt-5 max-w-xs text-base font-medium leading-7">
        En waar je absoluut niet aan begint.
      </motion.p>
    </div>
  );
}
...
function AgeGate() {
  return (
    <div className="pt-5">
      ...
      <Title>18+?</Title>
      <motion.p variants={childV} className="mt-7 text-lg leading-8" style={{ color: 'var(--text2)' }}>
        KinkSync gaat openlijk over BDSM, seks, fantasieën, verlangens en grenzen.
      </motion.p>
      <motion.p variants={childV} className="mt-5 text-base leading-7" style={{ color: 'var(--text2)' }}>
        Daar hoef je hier niet omheen te draaien.
      </motion.p>
      <motion.p variants={childV} className="mt-5 text-base font-semibold leading-7">
        Je moet wel 18 of ouder zijn.
      </motion.p>
    </div>
  );
}
  ...
  return (
    <div className="pt-2">
      ...
      <motion.p variants={childV} className="mt-6 text-base leading-7" style={{ color: 'var(--text2)' }}>
        Sommige dingen weet je meteen.<br />
        Bij andere wil je misschien eerst weten wat er precies bedoeld wordt.
      </motion.p>

      <motion.div
        variants={childV}
        className="mt-7 overflow-hidden rounded-2xl border px-5 py-6"
        style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}
      >
        <div className="flex items-center gap-3">
          <Sparkle size={20} aria-hidden="true" style={{ color: 'var(--accent)' }} />
        ...
        <div className="mt-5 flex flex-wrap gap-2.5">
          {statuses.map(([label, color]) => (
            <span
              ...
              style={{ borderColor: `color-mix(in srgb, ${color} 45%, var(--border))`, color }}
            >
              {label}
            </span>
          ))}
      ...
      <motion.p variants={childV} className="mt-7 text-base leading-7" style={{ color: 'var(--text2)' }}>
        ...
        Geen kinkier-dan-de-rest-score.
      </motion.p>
      <motion.p variants={childV} className="mt-5 text-base font-semibold leading-7">
        Gewoon waar jij staat.
      </motion.p>
    </div>
  );
}
      ...
      >
        {Array.from({ length: PIN_LENGTH }, (_, index) => (
          <div
            ...
            style={{ background: index < digits.length ? 'var(--accent)' : 'var(--border)' }}
          />
        ))}
      </motion.div>
      ...
      <motion.div variants={childV} className="grid grid-cols-3 gap-2.5">
        {PIN_KEYS.map((key, index) => (
          <button
            ...
            disabled={!key}
            className="focus-ring h-13 rounded-xl border text-xl font-semibold disabled:opacity-0"
            style={{
              background: key ? 'var(--surface2)' : 'transparent',
              borderColor: key ? 'var(--border)' : 'transparent',
  ...
  return (
    <div className="pt-5">
      ...
      {bioError && (
        <motion.p
          variants={childV}
          className="mt-6 whitespace-pre-line text-base leading-7"
...
function Finale() {
  return (
    <div className="pt-[clamp(2rem,5dvh,3.5rem)]">
      ...
      <motion.p variants={childV} className="mt-5 text-xl font-medium leading-8">
        Vanaf hier gaat het over jou.
      </motion.p>
      ...
      <motion.p variants={childV} className="mt-7 text-lg font-semibold leading-8">
        Kruip het konijnenhol in.
      </motion.p>
    </div>
  );
}
