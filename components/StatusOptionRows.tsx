      ...
      <div data-tour="pills" className="status-option-rows grid h-full min-h-[14.75rem] grid-rows-5 gap-1" role="group" aria-label="Status kiezen">
        {OPTIONS.map(({ status: s, label, hint, danger }) => {
          ...
          return (
            ...
            >
              <span className="status-option-copy grid h-full min-w-0 grid-cols-[1.25rem_auto_minmax(0,1fr)] items-center gap-3">
                <span
                  data-status-indicator={s}
                  aria-hidden="true"
                  className="status-option-indicator inline-flex h-5 w-5 items-center justify-center rounded-full transition-[background-color,border-color,box-shadow,color] duration-100 motion-reduce:transition-none"
                  style={active
                    ? {
                        color: colour,
